...
public class ElementXml implements Serializable {
	...
	public ElementXml addNode(String tagName, Object value) {
		return addNode(tagName, value, true);
	}
	public ElementXml addNode(String tagName, Object value, boolean required) {
		if(value == null) {
			if(required)throw new RuntimeException("Valor requerido para tag '" + tagName + "' não informado");
			return null;
		}
		Element child = element.getOwnerDocument().createElement(tagName.trim());
		element.appendChild(child);
	...
	public List<String> findChildValues(String query) {
		List<ElementXml> childrenNode = findChildNodes(query);
